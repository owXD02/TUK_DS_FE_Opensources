...
| 문서 | 언제 보나 |
|---|---|
| **README.md** (이 문서) | 모듈별 함수 사용법 + 시험 유형별 조립 레시피 |
| **C_기초문법.md** | C가 가물가물할 때 — 구조체·포인터·malloc·파일 입출력 등 |
| **TROUBLESHOOTING.md** | 컴파일/실행 에러가 났을 때 (에러 코드별 대처) |
| **../풀이/** , **../드론풀이/** | 예상문제 20 + 드론 20 모범답안 (조립 예시 모음) |

> 처음이면 **C_기초문법.md → 이 README의 1~4장** 순서로 훑고, 막히면 TROUBLESHOOTING.md.
...
| `search.h/.c` | **탐색**: 순차 / 정렬 순차 / 이진(반복·재귀) / 색인 + 비교 횟수 | "정렬 후 이진 탐색, 비교 횟수" |
...
| `arrayDeque.h/.c` | **순차 데크** (배열·원형, int, 양쪽 삽입/삭제) | "배열로 덱을 구현하라" |
| `deque.h/.c` | **연결 데크(Deque)** (이중 연결, int, 양쪽 삽입/삭제) | "연결 리스트로 덱을 구현하라" |
...
| `queuing.h/.c` | **큐잉 이론 M/M/1 공식** (ρ, L, Lq, W, Wq, P0, Pn) | "공식으로 평균 대기 시간 계산" |
| `linkedList.h/.c` | **범용 단일 연결 리스트** (임의 위치 삽입/삭제, 탐색, 역순) | "연결 리스트로 ~하라" |
...
| `main.c` | 메뉴식 데모(16종) = 유형별 답안 뼈대 모음 | 시험 때 이 파일을 고쳐 쓰기 |
...
| 원형 큐 | `cQueue` |
| 연결 큐 | `linkedQueue` |
| 순차 데크 (배열·원형) | `arrayDeque` |
| 연결 데크 | `deque` (정수 노드), `xyList` (좌표) |
| 작업 큐 | `jobQueue` (Job 구조체 + 큐) |
| 큐잉 이론 (시뮬레이션) | `jobQueue`의 `runQueueSim` (난수 도착, 평균 대기 시간) |
| 큐잉 이론 (M/M/1 공식) | `queuing` (ρ, L, Lq, W, Wq, P0, Pn) |
| 신뢰성/가용성 (MTBF/MTTR) | `reliability` |
| 탐색 (순차/이진/색인) | `search` |
| 연결 리스트 (임의 삽입/삭제/역순) | `linkedList` |
...
### 큐 & 덱 — sQueue / cQueue / linkedQueue / arrayDeque / deque
```c
...
Deque* DQ = createDeque();          // 연결 데크: 양쪽 끝에서 삽입/삭제
...
ADeque* ADQ = createADeque();       // 순차 데크(배열·원형): 같은 연산, 배열 구현
aqInsertFront(ADQ, 1);  aqInsertRear(ADQ, 2);
aqDeleteFront(ADQ);  aqDeleteRear(ADQ);  aqPeekFront(ADQ);  aqPeekRear(ADQ);
aqCount(ADQ);  printADeque(ADQ, NULL);  free(ADQ);
```
...
### queuing — 큐잉 이론 M/M/1 공식
```c
printMM1(2.0, 3.0, NULL);            // λ=2, μ=3 한 번에 보고서 (ρ,L,Lq,W,Wq,P0)
double rho = calcRho(2.0, 3.0);      // 0.667   double L = calcL(2.0,3.0);  // 2.0
double w   = calcW(2.0, 3.0);        // 1.0     double wq = calcWq(2.0,3.0);// 0.667
// 안정 조건 λ<μ. μ<=λ이면 L/W/Lq/Wq는 -1 반환(불안정)
```
> 시뮬레이션으로 구하려면 jobQueue의 runQueueSim, 공식으로 바로 구하려면 queuing.

### search — 탐색 (비교 횟수 포함)
```c
int cmp;
int idx = sequentialSearch(a, n, key, &cmp);     // 정렬 무관
quickSort(a, n, 1);                              // 이진/색인은 정렬 먼저!
idx = binarySearch(a, n, key, &cmp);             // 반복식
cmp = 0; idx = binarySearchRecursive(a, 0, n-1, key, &cmp);  // 재귀식(cmp 초기화)
idx = indexSearch(a, n, key, 4, &cmp);           // 블록 크기 4 색인 탐색
// cmp가 필요 없으면 NULL 전달 가능
```

### linkedList — 범용 단일 연결 리스트
```c
LinkedList* L = createList();
insertFirst(L, 5);  insertLast(L, 20);  insertAt(L, 1, 10);  // [5 10 20]
searchList(L, 10);            // 인덱스 반환(없으면 -1)
deleteValue(L, 10);          // 값으로 삭제
reverseList(L);              // 역순 뒤집기
printList(L, NULL);  listLength(L);  freeList(L);
```
